...
const main = ()=>{
  ...
  app.get('/chats/metadata/:type/:id', (req, res)=>{
    ...
    const selectedChat = (type === 'group') ? databases.eChats.getGcById(id) : databases.eChats.getPcById(id);
    if (selectedChat) {
      res.status(200).json({
        success: true,
  ...
  app.post('/chats/message/:type/:chatId', (req, res)=>{
    ...
    if (typeof(msgIdRecived) === 'object' && userId) {
      const selectedChat = (type === 'group') ? (databases.eChats.getGcById(chatId)) : (databases.eChats.getPcById(chatId));
      if (selectedChat) {
        const listOfParticipantsId = selectedChat.GroupParticipants.map(user => {
          return user.id
        })
        if (listOfParticipantsId.includes(userId)) {
          let filteredMessage = Utility.oop.copy(selectedChat.Messages)
          for (let msgId of msgIdRecived) {
